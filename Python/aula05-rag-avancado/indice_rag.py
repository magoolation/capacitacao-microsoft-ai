...
`buscar`: o modo ganhou a variante HIBRIDA_COM_RERANKING (para medir o que o
ranqueador semântico ganha) e o `top_k` deixou de ser um detalhe com valor
padrão e virou o parâmetro que a aula inteira gira em torno.
"""
...
class Modo(Enum):
    ...
    KEYWORD = "keyword"                        # só BM25: acerta o literal, erra o sinônimo
    VETORIAL = "vetorial"                      # só vetorial: acerta o sinônimo, erra o código exato
    HIBRIDA = "hibrida"                        # vetorial + BM25, sem reordenação semântica
    HIBRIDA_COM_RERANKING = "hibrida+rerank"   # híbrida com o ranqueador semântico por cima
    # HIBRIDA_COM_RERANKING é o modo padrão desta aula, e a opção 6 do menu
    # existe para medir quanto ele muda.
...
class IndiceRag:
    ...
    # 5. Recuperação
    # -------------------------------------------------------------------------
    def buscar(self, pergunta: str, modo: Modo, top_k: int) -> list[tuple[Chunk, float | None]]:
        parametros: dict = {
            "top": top_k,
        ...
        # A metade semântica só existe no modo híbrido com reranking.
        if modo is Modo.HIBRIDA_COM_RERANKING:
